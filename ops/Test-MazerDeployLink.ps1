...
if ([string]::IsNullOrWhiteSpace($ConfigPath)) {
  $ConfigPath = Join-Path 'config' 'mazer-deploy.identity.json'
}
...
if (-not (Test-Path -LiteralPath $resolvedConfigPath)) {
  throw ("Mazer deploy identity config was not found at {0}." -f $resolvedConfigPath)
}
...
if ($repoGitExitCode -ne 0 -or [string]::IsNullOrWhiteSpace($normalizedRepoGitTopLevel) -or $normalizedRepoGitTopLevel -ne $normalizedRepoPath) {
  ...
  Write-Host 'The target path is not a real standalone Mazer repo boundary.' -ForegroundColor Yellow
  Write-Host ''
  ...
  Write-Host 'Expected: the Mazer repo path must be its own git toplevel before any deploy lane can run.'
  exit 1
}
...
if ($linkedTeamId -ne $expectedTeamId) {
  $mismatches.Add('.vercel/project.json orgId does not match the required Mazer Vercel team ID')
  $diagnoses.Add('Wrong account or team link: the local .vercel metadata points at a different Vercel team ID than the canonical Mazer lane.')
}

if ($linkedProjectId -ne $expectedProjectId) {
  $mismatches.Add('.vercel/project.json projectId does not match the required Mazer Vercel project ID')
...
if ($linkedProjectName -ne $expectedProject) {
  $mismatches.Add('.vercel/project.json projectName does not match the required current Mazer project name')
  if ($linkedTeamId -eq $expectedTeamId -and $linkedProjectId -eq $expectedProjectId) {
    $diagnoses.Add('Rename drift: immutable IDs match the canonical Mazer project, but the current human-readable project name is stale.')
  }
}

if ($mismatches.Count -gt 0) {
  Write-Host 'Mazer deploy blocked before Vercel.' -ForegroundColor Red
  Write-Host 'Preview and production deploys must target the canonical Mazer Vercel identity first.' -ForegroundColor Yellow
  Write-Host ''
  Write-Host ("Expected Vercel identity: team {0} / project {1} ({2}/{3})" -f $expectedTeamId, $expectedProjectId, $expectedScope, $expectedProject)
  Write-Host ("Repo path: {0}" -f $resolvedRepoPath)
  Write-Host ("Deploy identity config: {0}" -f $resolvedConfigPath)
...
Write-Host ("Mazer deploy link preflight passed for team {0} / project {1} ({2}/{3})." -f $expectedTeamId, $expectedProjectId, $expectedScope, $expectedProject) -ForegroundColor Green
Write-Host ("Deploy identity config: {0}" -f $resolvedConfigPath)
Write-Host ("Local project.json path: {0}" -f $projectJsonPath)
