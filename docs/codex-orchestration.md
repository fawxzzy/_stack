...
- bounded queue-or-registry helper commands remain explicit-input; retained-state wrappers stay read-only, the worker-artifact-emission wrapper may emit only admitted assignment or running/completed status artifacts into `_stack` repo-local Codex log directories, the execution-bridge-artifacts wrapper may emit only one admitted receipt-backed execution status update plus one bridge record plus one explicit receipt output root while still avoiding queue-state or dispatch behavior, the queue-drop-emission wrapper may emit only one admitted Markdown drop into `_stack` `queue/pending/` without implying claim-state or launch behavior, the launch-or-dispatch wrapper may stage only one explicit pending drop into one bounded `_stack` inbox home and emit only one worker assignment plus running-status pair without implying completion or queue-state advancement, the claim-state-movement wrapper may move only one explicit pending drop into one bounded `_stack` `queue/claimed/` home for one active worker without implying completion or done-state advancement, the done-state-closure wrapper may move only one explicit claimed drop into one bounded `_stack` `queue/done/` home for one completed worker without implying merge closure, resume closure, or publication proof, the merge-request-artifact-behavior wrapper may emit only one explicit merge-request artifact into one admitted `_stack` repo-local Codex log directory without implying paused-status, merger-assignment, or resume-ready behavior, the paused-status-artifact-behavior wrapper may emit only paused worker status artifacts for one explicit merge request without implying merger-assignment or resume-ready behavior, the merger-assignment-artifact-behavior wrapper may emit only one merger worker assignment plus its bounded merge prompt/context artifacts for one explicit merge request after paused statuses already exist without implying resume-ready behavior, the resume-context-artifact-behavior wrapper may emit only per-worker resume-context artifacts for one explicit merge request after paused statuses and merger assignment already exist without implying merge-completion or root-resume behavior, and the merge-completion-behavior wrapper may emit only one merge completion artifact for one explicit merge request after paused statuses, merger assignment, and resume-context artifacts already exist without implying root-resume behavior
