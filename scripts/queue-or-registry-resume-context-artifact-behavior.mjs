...
const COMMAND_ID = "stack queue-or-registry resume-context-artifact-behavior";
const WORKSPACE_ROOT_ENV = "STACK_QUEUE_OR_REGISTRY_RESUME_CONTEXT_ARTIFACT_BEHAVIOR_WORKSPACE_ROOT";
const POWERSHELL_ENV = "STACK_QUEUE_OR_REGISTRY_RESUME_CONTEXT_ARTIFACT_BEHAVIOR_POWERSHELL";
const BRIDGE_SCRIPT_REF = "repos/_stack/ops/stack/Invoke-QueueOrRegistryResumeContextArtifactBehavior.ps1";
const STACK_LOCK_REF = "stack.lock.yaml";
const MERGE_REQUEST_CONTRACT_VERSION = "atlas.worker.merge-request.v1";
const RESULT_CLASS = "resume-context-artifacts-emitted";
const ROUTING_NOTE =
...
const ROUTING_NOTES = Object.freeze({
  invalidMergeRequest: "repair the explicit merge-request artifact before resume-context emission",
  invalidArtifactSearchRoot: "repair the explicit artifact-search-root before resume-context emission",
  lineageMismatch: "repair the explicit merge-request lineage mismatch before wider orchestration claims",
  builderFailed: "repair the admitted _stack resume-context consumer before wider orchestration claims",
  malformedResumeOutput: "repair the admitted _stack resume-context output contract before wider orchestration claims"
});
...
function buildSuccess({
  ...
  return {
    ...
    payload: {
      resume_context_artifacts: payload
    }
  };
}
...
function parseBridgeOutput(bridgeResult) {
  if (!bridgeResult.ok) {
    return {
      ok: false,
      kind: "builder-failed",
      message: "The admitted _stack resume-context consumer failed before emission could complete."
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      kind: "malformed-resume-output",
      message: "The admitted _stack resume-context consumer emitted non-json output."
    };
  }
  ...
  if (
    ...
    return {
      ok: false,
      kind: "malformed-resume-output",
      message: "The admitted _stack resume-context consumer omitted the expected resume-context output surface."
    };
  }
...
export async function runQueueOrRegistryResumeContextArtifactBehaviorCommand(argv, dependencies = {}) {
  ...
  if (!parsedBridge.ok) {
    const failureCode = parsedBridge.kind === "builder-failed" ? "builder-failed" : "malformed-resume-output";
    const routingNote =
      parsedBridge.kind === "builder-failed" ? ROUTING_NOTES.builderFailed : ROUTING_NOTES.malformedResumeOutput;
    return {
      ok: false,
      report: buildFailure({
        failureCode,
...
function usage(scriptName) {
  return [
    ...
    "",
    "Emits per-worker resume-context artifacts for one explicit merge request only after paused statuses and merger assignment already exist.",
    "No-merge-completion/no-root-resume guard: this packet may admit future implementation of one explicit resume-context wrapper input parser, one root-relative path normalization layer for merge-request or artifact-search-root refs, one exact merge-request validator, one admitted _stack resume-context consumer invocation only, one bounded resume-context report renderer, and one fail-closed invalid-merge-request or lineage-mismatch or invalid-artifact-search-root handler, but it may not emit merge-completion artifacts, consume resume contexts into merge completion, mint root-owned resume request or dispatch artifacts, inspect broader queue history, mutate atlas-book or lock surfaces, mutate owner-repo surfaces outside the admitted resume-context artifact home, or imply lifecycle advancement, deploy readiness, or publication proof."
  ].join("\n");
}
...
async function main(argv) {
  ...
  const result = await runQueueOrRegistryResumeContextArtifactBehaviorCommand(argv);
  const parsed = parseArgs(argv);
