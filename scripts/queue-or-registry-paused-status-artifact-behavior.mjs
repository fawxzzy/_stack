...
const COMMAND_ID = "stack queue-or-registry paused-status-artifact-behavior";
const WORKSPACE_ROOT_ENV = "STACK_QUEUE_OR_REGISTRY_PAUSED_STATUS_ARTIFACT_BEHAVIOR_WORKSPACE_ROOT";
const POWERSHELL_ENV = "STACK_QUEUE_OR_REGISTRY_PAUSED_STATUS_ARTIFACT_BEHAVIOR_POWERSHELL";
const BRIDGE_SCRIPT_REF = "repos/_stack/ops/stack/Invoke-QueueOrRegistryPausedStatusArtifactBehavior.ps1";
const STACK_LOCK_REF = "stack.lock.yaml";
const MERGE_REQUEST_CONTRACT_VERSION = "atlas.worker.merge-request.v1";
const RESULT_CLASS = "paused-status-artifacts-emitted";
const ROUTING_NOTE =
...
const ROUTING_NOTES = Object.freeze({
  invalidMergeRequest: "repair the explicit merge-request artifact before paused-status emission",
  invalidArtifactSearchRoot: "repair the explicit artifact-search-root before paused-status emission",
  lineageMismatch: "repair the explicit merge-request lineage mismatch before wider orchestration claims",
  builderFailed: "repair the admitted _stack paused-status consumer before wider orchestration claims",
  malformedPauseOutput: "repair the admitted _stack paused-status output contract before wider orchestration claims"
});
...
function buildSuccess({
  ...
  return {
    ...
    pause_status_refs: pauseStatusRefs,
    stack_lock_digest: stackLockDigest,
    result_class: RESULT_CLASS,
    routing_note: ROUTING_NOTE,
    payload: {
      paused_status_artifacts: payload
    }
  };
}
...
function renderText(result) {
  if (result.ok) {
    return [
      ...
      `pause_status_refs=${JSON.stringify(result.report.pause_status_refs)}`,
...
function parseBridgeOutput(bridgeResult) {
  if (!bridgeResult.ok) {
    return {
      ok: false,
      kind: "builder-failed",
      message: "The admitted _stack paused-status consumer failed before emission could complete."
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      kind: "malformed-pause-output",
      message: "The admitted _stack paused-status consumer emitted non-json output."
    };
  }

  if (
    !isRecord(payload)
    || !Array.isArray(payload.pause_status_refs)
    || payload.pause_status_refs.length < 1
    || !payload.pause_status_refs.every((item) => isNonEmptyString(item))
    || !isRecord(payload.payload)
    || !Array.isArray(payload.payload.pause_statuses)
    || payload.payload.pause_statuses.length !== payload.pause_status_refs.length
  ) {
    return {
      ok: false,
      kind: "malformed-pause-output",
      message: "The admitted _stack paused-status consumer omitted the expected paused-status output surface."
    };
  }
...
export async function runQueueOrRegistryPausedStatusArtifactBehaviorCommand(argv, dependencies = {}) {
  ...
  if (!parsedBridge.ok) {
    const failureCode = parsedBridge.kind === "builder-failed" ? "builder-failed" : "malformed-pause-output";
    const routingNote =
      parsedBridge.kind === "builder-failed" ? ROUTING_NOTES.builderFailed : ROUTING_NOTES.malformedPauseOutput;
    return {
      ok: false,
      report: buildFailure({
        failureCode,
  ...
  return {
    ...
    report: buildSuccess({
      ...
      pauseStatusRefs: parsedBridge.payload.pause_status_refs,
      stackLockDigest: stackLockDigestResult.stackLockDigest,
...
function usage(scriptName) {
  return [
    ...
    "",
    "Emits paused worker status artifacts for one explicit merge request only.",
    "No-merge/no-resume guard: this packet may admit future implementation of one explicit paused-status wrapper input parser, one root-relative path normalization layer for merge-request or artifact-search-root refs, one exact merge-request validator, one admitted _stack paused-status consumer invocation only, one bounded pause-status report renderer, and one fail-closed invalid-merge-request or lineage-mismatch or invalid-artifact-search-root handler, but it may not emit merger-assignment or resume-context artifacts, consume paused workers into merge completion, inspect broader queue history, mutate atlas-book or lock surfaces, mutate owner-repo surfaces outside the admitted status-artifact home, or imply lifecycle advancement, deploy readiness, or publication proof."
  ].join("\n");
}
...
async function main(argv) {
  ...
  const result = await runQueueOrRegistryPausedStatusArtifactBehaviorCommand(argv);
  const parsed = parseArgs(argv);
