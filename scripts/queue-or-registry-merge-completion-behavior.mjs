...
const COMMAND_ID = "stack queue-or-registry merge-completion-behavior";
const WORKSPACE_ROOT_ENV = "STACK_QUEUE_OR_REGISTRY_MERGE_COMPLETION_BEHAVIOR_WORKSPACE_ROOT";
const POWERSHELL_ENV = "STACK_QUEUE_OR_REGISTRY_MERGE_COMPLETION_BEHAVIOR_POWERSHELL";
const BRIDGE_SCRIPT_REF = "repos/_stack/ops/stack/Invoke-QueueOrRegistryMergeCompletionBehavior.ps1";
const STACK_LOCK_REF = "stack.lock.yaml";
const MERGE_REQUEST_CONTRACT_VERSION = "atlas.worker.merge-request.v1";
const RESULT_CLASS = "merge-completion-artifact-emitted";
const ROUTING_NOTE =
...
const ROUTING_NOTES = Object.freeze({
  invalidMergeRequest: "repair the explicit merge-request artifact before merge-completion emission",
  invalidArtifactSearchRoot: "repair the explicit artifact-search-root before merge-completion emission",
  lineageMismatch: "repair the explicit merge-request lineage mismatch before wider orchestration claims",
  builderFailed: "repair the admitted _stack merge-completion consumer before wider orchestration claims",
  malformedCompletionOutput: "repair the admitted _stack merge-completion output contract before wider orchestration claims"
});
...
function buildSuccess({
  ...
  return {
    ...
    resume_context_refs: resumeContextRefs,
    ...
    payload: {
      merge_completion_artifact: payload
    }
  };
}
...
function renderText(result) {
  if (result.ok) {
    return [
      ...
      `completion_ref=${result.report.completion_ref}`,
      `resume_context_refs=${JSON.stringify(result.report.resume_context_refs)}`,
      ...
      `merge_handoff_ref=${result.report.merge_handoff_ref}`,
...
function parseBridgeOutput(bridgeResult) {
  if (!bridgeResult.ok) {
    return {
      ok: false,
      kind: "builder-failed",
      message: "The admitted _stack merge-completion consumer failed before emission could complete."
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      kind: "malformed-completion-output",
      message: "The admitted _stack merge-completion consumer emitted non-json output."
    };
  }

  if (
    !isRecord(payload)
    || !isNonEmptyString(payload.completion_ref)
    || !Array.isArray(payload.resume_context_refs)
    || payload.resume_context_refs.length < 1
    || !payload.resume_context_refs.every((item) => isNonEmptyString(item))
    ...
    || !isNonEmptyString(payload.merge_handoff_ref)
    || !isRecord(payload.payload)
    || !Array.isArray(payload.payload.resume_contexts)
    || payload.payload.resume_contexts.length !== payload.resume_context_refs.length
  ) {
    return {
      ok: false,
      kind: "malformed-completion-output",
      message: "The admitted _stack merge-completion consumer omitted the expected completion output surface."
    };
  }
...
export async function runQueueOrRegistryMergeCompletionBehaviorCommand(argv, dependencies = {}) {
  ...
  if (!parsedBridge.ok) {
    const failureCode = parsedBridge.kind === "builder-failed" ? "builder-failed" : "malformed-completion-output";
    const routingNote =
      parsedBridge.kind === "builder-failed" ? ROUTING_NOTES.builderFailed : ROUTING_NOTES.malformedCompletionOutput;
    return {
      ok: false,
      report: buildFailure({
        failureCode,
  ...
  return {
    ...
    report: buildSuccess({
      ...
      completionRef: parsedBridge.payload.completion_ref,
      resumeContextRefs: parsedBridge.payload.resume_context_refs,
      ...
      mergeHandoffRef: parsedBridge.payload.merge_handoff_ref,
      stackLockDigest: stackLockDigestResult.stackLockDigest,
...
function usage(scriptName) {
  return [
    ...
    "",
    "Emits one merge completion artifact for one explicit merge request only after paused statuses, merger assignment, and resume-context artifacts already exist.",
    "No-root-resume guard: this packet may admit future implementation of one explicit merge-completion wrapper input parser, one root-relative path normalization layer for merge-request or artifact-search-root refs, one exact merge-request validator, one admitted _stack merge-completion consumer invocation only, one bounded merge-completion report renderer, and one fail-closed invalid-merge-request or lineage-mismatch or invalid-artifact-search-root handler, but it may not mint root-owned resume request or dispatch artifacts, inspect broader queue history, mutate atlas-book or lock surfaces, mutate owner-repo surfaces outside the admitted merge completion artifact home, or imply lifecycle advancement, deploy readiness, or publication proof."
  ].join("\n");
}
...
async function main(argv) {
  ...
  const result = await runQueueOrRegistryMergeCompletionBehaviorCommand(argv);
  const parsed = parseArgs(argv);
